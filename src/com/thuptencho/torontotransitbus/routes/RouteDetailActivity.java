...
public class RouteDetailActivity extends Activity {
	...
	private BroadcastReceiver mMessageReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			try {
				cursor = context.getContentResolver().query(Constants.CONTENT_URI_ROUTE,
				...
				cursor1 = context.getContentResolver().query(Constants.CONTENT_URI_DIRECTION, 
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Uri contentUriRouteSingle = ContentUris.withAppendedId(Constants.CONTENT_URI_ROUTE, position);
		...
		intent.putExtra(Constants.URL_STRING_KEY, RestClient.getRestUrlForRouteDetail(routeTag));
		startService(intent);

		IntentFilter ifilter = new IntentFilter(Constants.BROADCAST_ROUTE_DETAIL_UPDATED);
