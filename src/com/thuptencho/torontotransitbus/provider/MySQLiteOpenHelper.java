...
//@formatter:off;
	...
	private static final String CREATE_TABLE_DIRECTIONS = 	"CREATE  TABLE  Directions ( " + 
			...
			"foreign key(route__tag) references routes(tag)); ";
	...
	private static final String CREATE_TABLE_ROUTES_STOPS = "CREATE TABLE Routes_Stops ("+
			"_id INTEGER PRIMARY KEY AUTOINCREMENT, "+
			"route__tag VARCHAR(45) NOT NULL, " + 
			"stop__tag VARCHAR(45) NOT NULL, " +
			"foreign key(route__tag) references Routes(tag), " +
			"foreign key(stop__tag) references Stops(tag)); ";
	
	private static final String CREATE_TABLE_DIRECTIONS_STOPS = "CREATE TABLE Directions_Stops ("+
			"_id INTEGER PRIMARY KEY AUTOINCREMENT , "+
			"direction__tag VARCHAR(45) NOT NULL, " + 
			"stop__tag VARCHAR(45) NOT NULL, " +
			"foreign key (direction__tag) references Directions(tag), " +
			"foreign key (stop__tag) references Stops(tag)); ";
	...
	private static final String CREATE_TABLE_POINTS = 	"CREATE  TABLE  Points (" + 
			...
			"foreign key (Paths__id) references Paths(_id), "+
...
//@formatter:on;	
	...
	public MySQLiteOpenHelper(Context context, String name, CursorFactory factory, int databaseVersion) {
	...
	public void onCreate(SQLiteDatabase db) {
		...
		db.execSQL(CREATE_TABLE_ROUTES_STOPS);
	...
	public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
		...
		db.execSQL("drop table if exists Routes_Stops");
		db.execSQL("drop table if exists Routes_Directions");
		db.execSQL("drop table if exists Directions_Stops");
